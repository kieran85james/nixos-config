...
with lib; let
  ...
  myAliases = {
    ...
    "wget" = "wget -c";
    "yt-dlp" = "nix shell nixpkgs#yt-dlp --command yt-dlp -o \"$(xdg-user-dir DOWNLOAD)/%(playlist)s/%(playlist_index)02d - %(title)s.%(ext)s\"";
...
in {
  options.features.cli.bash.enable =
    mkEnableOption "Enable extended bash configuration";
